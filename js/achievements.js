class AchievementsManager {
    ...
    checkAchievementRequirement(achievement, newEntry, allEntries, streaks) {
        ...
        switch (req.type) {
            ...
            case 'total_days':
                // Return all players who have participated in enough entries
                ...
                return allEntries.length >= req.value ? Array.from(participatingPlayers) : [];
    ...
    async getAllPlayers() {
        ...
        // This queries the achievements/games tables to find all registered players
        try {
            const response = await fetch('/api/stats?type=all');
            if (!response.ok) return [];
            const data = await response.json();

            // Extract unique player names from streaks
    ...
    checkWeeklySweep(allEntries) {
        ...
        for (let i = 0; i <= sortedEntries.length - 7; i++) {
            ...
            if (isConsecutive) {
                // For personal milestones, check if user completed all weekend puzzles
                ...
                // Return players who completed all 7 days
                return Object.keys(playerCompletions).filter(p => playerCompletions[p] === 7);
            }
        }
    ...
    checkTimeUnder(req, entry) {
        // Check all players in entry dynamically
        const players = Object.keys(entry).filter(key => typeof entry[key] === 'object' && entry[key].times);
    ...
    checkDailyScore(requiredScore, entry) {
        const players = [];
        // Check score for all players in entry
    ...
    checkComebackWin(lossStreak, allEntries) {
        ...
        // For personal milestones: check if player had low scores and then achieved high score
        const latestEntry = recentEntries[recentEntries.length - 1];
        const previousEntries = recentEntries.slice(0, -1);
        const players = [];

        // Check each player for comeback pattern (low scores → high score)
        Object.keys(latestEntry).filter(k => typeof latestEntry[k] === 'object').forEach(player => {
            // Did this player have consistently low scores before?
            const hadLowScores = previousEntries.every(entry => {
                return entry[player]?.scores?.total && entry[player].scores.total < 500;
            });

            // Did they achieve a high score in the latest entry?
            const hasHighScore = latestEntry[player]?.scores?.total > 1000;

            if (hadLowScores && hasHighScore) {
                players.push(player);
            }
        });
    ...
    checkWeekendSweep(allEntries) {
        ...
        const players = [];
        // Check if player completed both weekend days
    ...
    checkCloseWin(req, entry) {
        // For personal milestones, check if user hit exact score threshold
        const players = [];
        Object.keys(entry).filter(k => typeof entry[k] === 'object').forEach(player => {
            if (entry[player]?.scores?.total === req.value) {
                players.push(player);
            }
        });
        return players;
    ...
    checkDominatingWin(req, entry) {
        const players = [];
        // Check if player achieved dominating score (high threshold)
        Object.keys(entry).filter(k => typeof entry[k] === 'object').forEach(player => {
            if (entry[player]?.scores?.total >= req.value * 1000) { // High score threshold
                players.push(player);
            }
        });
        return players;
    ...
    checkWinWithDnf(req, entry) {
        ...
        return players.filter(player => {
            ...
            // For personal milestones, check if completed all puzzles
            const completedAll = ['easy', 'medium', 'hard'].every(d => entry[player].times[d] !== null);
    ...
    updateAchievementsSummary() {
        ...
        // Update DOM elements
        // Update achievement counts for all players dynamically
