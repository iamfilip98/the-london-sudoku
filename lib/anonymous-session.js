...
async function migrateAnonymousToUser(userId) {
  ...
  if (progress.completions.length === 0 && progress.achievements.length === 0) {
    console.log('No anonymous progress to migrate');
    return { migrated: 0 };
  }

  console.log(`Migrating anonymous progress for user ${userId}...`);
  console.log(`- Completions: ${progress.completions.length}`);
  ...
  try {
    ...
    for (const completion of progress.completions) {
      try {
        const response = await fetch('/api/import-completion', {
          method: 'POST',
          headers: {
    ...
    for (const achievement of progress.achievements) {
      try {
        const response = await fetch('/api/import-achievement', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${sessionStorage.getItem('clerk_token')}`
        ...
        console.error('Failed to migrate achievement:', error);
      }
    }
    ...
    console.log(`- Migrated ${migratedCompletions} completions`);
    console.log(`- Migrated ${migratedAchievements} achievements`);
...
function clearAnonymousData() {
  ...
  localStorage.removeItem('anonymous_progress');
  localStorage.removeItem('last_signup_prompt');
  console.log('🗑️ Anonymous data cleared');
}
