...
async function processSeasonEnd() {
    ...
    try {
        ...
        for (const season of activeSeasons) {
            ...
            // Calculate max possible points for this season (7 days * max daily points)
            // Assuming max daily points is based on completing all 3 difficulties perfectly
            const maxPossiblePoints = 7 * (100 + 200 + 300); // Easy + Medium + Hard max scores
            ...
            for (let i = 0; i < rankings.rows.length; i++) {
                ...
                // Calculate rank percentile (what % of players did they beat)
                const rankPercentile = ((totalMembers - rank + 1) / totalMembers) * 100;

                // Calculate winning margin (only for rank 1)
                let winningMargin = null;
                if (rank === 1 && rankings.rows.length > 1) {
                    const secondPlacePoints = rankings.rows[1].season_points || 0;
                    winningMargin = member.season_points - secondPlacePoints;
                }
                ...
                // Save season result with enhanced tracking
                await client.query(`
                    INSERT INTO league_season_results
                    (season_id, user_id, final_points, final_rank, outcome, new_tier, rank_percentile, winning_margin, max_possible_points)
                    VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
                `, [season.id, member.user_id, member.season_points, rank, outcome, newTier, rankPercentile, winningMargin, maxPossiblePoints]);
