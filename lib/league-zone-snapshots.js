...
async function takeLeagueSnapshots() {
    ...
    try {
        ...
        console.log(`Taking snapshots for ${activeSeasons.rows.length} active seasons...`);
        ...
        for (const season of activeSeasons.rows) {
            ...
            if (rankings.rows.length === 0) {
                console.log(`No members in ${season.tier} league, skipping`);
                continue;
            }
            ...
            console.log(`Created ${rankings.rows.length} snapshots for ${season.tier} league`);
        }

        await client.query('COMMIT');

        console.log(`League snapshots complete: ${totalSnapshots} snapshots created`);
        return {
            success: true,
            date: snapshotDate,
        ...
        console.error('Failed to take league snapshots:', error);
        throw error;
    } finally {
        client.release();
    }
