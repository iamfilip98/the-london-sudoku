...
const difficultyResultSchema = z.object({
...
const saveEntrySchema = z.object({
  date: date,
  times: z.record(difficulty, z.number().int().min(0).optional()).optional(),
  scores: z.record(difficulty, z.number().min(0).optional()).optional(),
  errors: z.record(difficulty, z.number().int().min(0).optional()).optional(),
  hints: z.record(difficulty, z.number().int().min(0).optional()).optional(),
  dnf: z.record(difficulty, z.boolean().optional()).optional(),
  // Support for legacy data structure during migration
  easy: difficultyResultSchema,
  medium: difficultyResultSchema,
  hard: difficultyResultSchema
});
